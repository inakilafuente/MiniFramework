...
(function ($) {
	...
	Calendar.prototype = {
		...
		_initializeOptions: function (opt) {
			...
			this.options = {
				...
				style: opt.style == 'background' || opt.style == 'border' || opt.style == 'custom' || opt.style == 'acciona'? opt.style : 'border',
		...
		_renderDataSourceDay: function (elt, currentDate, events) {
			switch (this.options.style) {
				...
				case 'calendario_acciona':
					elt.parent().css('color', '#FF0000');
					...
					if (events[events.length - 1].startDate.getTime() == currentTime) {
						//SALMON
						if (events[events.length - 1].tipoFestivo == 'No Laborable') {
							var colorFondo = '#FF9999';

						}

						//AZUL
						if (events[events.length - 1].tipoFestivo == 'Autonomico') {
							var colorFondo = '#2c8fc9';
						}

						//VERDE
						if (events[events.length - 1].tipoFestivo == 'Local') {
							var colorFondo = '#9CB703';
						}

						// ROJO
						if (events[events.length - 1].tipoFestivo == 'Nacional') {
							var colorFondo = '#ff4a32';
						}
						//elt.css('color', '#000000');
						elt.css('background-color', colorFondo);
						events[events.length - 1].color = colorFondo;
						...
						if (events[events.length - 1].tipoFestivo == 'No Laborable') {
							elt.css('color', '#000000');
							elt.css('background-color', '#FF9999');
						}
				...
				case 'acciona':
					...
					if (weight > 0) {
						...
						//  Azul celeste // verde   //amarillo //rojo   // violeta  //azul verdoso
						for (var i = 0; i < events.length; i++) {
							colorFondo = "";
							if (boxShadow != '') {
								boxShadow += ",";
							}
							if (events[i].tipoFestivo == 'No Laborable') {
								var colorFondo = colors[4];
								events[i].color = colorFondo;
								elt.css('background-color', colors[4]);
								var colorFondo = 'transparent';
								//events[i].color = '';
							}

							//AZUL
							else if (events[i].tipoFestivo == 'Autonomico') {
								//var colorFondo = colors[0];
								//events[i].color = colorFondo;

								var colorFondo = colors[0];
								events[i].color = colorFondo;
								elt.css('background-color', colors[0]);
								var colorFondo = 'transparent';
							}

							//VERDE
							else if (events[i].tipoFestivo == 'Local') {
								//var colorFondo = colors[1];
								//events[i].color = colorFondo;

								var colorFondo = colors[1];
								events[i].color = colorFondo;
								elt.css('background-color', colors[1]);
								var colorFondo = 'transparent';
							}

							// ROJO
							else if (events[i].tipoFestivo == 'Nacional') {
								//var colorFondo = colors[3];
								//events[i].color = colorFondo;

								var colorFondo = colors[3];
								events[i].color = colorFondo;
								elt.css('background-color', colors[3]);
								var colorFondo = 'transparent';

							}
							else if (events[i].tipoFestivo == 'No Festivo' && events[i].horarioVerano == 1) {
								var colorFondo = colors[2];
								events[i].color = colorFondo;
							}
							else if (events[i].tipoFestivo == 'No Festivo' && events[i].horarioEspecial == 1) {
								//elt.css('background-color', colors[5]);
								//var colorFondo = 'transparent';
								//events[i].color = '';
							...
							boxShadow += 'inset 0 -' + (parseInt(i) + 1) * weight + 'px 0 0 ' + colorFondo;
						}
		...
		_openContextMenu: function (elt) {
			...
			//CAMBIO DE FOR PARA NUESTRO ENTORNO
			//for(var i in events) {
			for (var i = 0; i < events.length; i++) {
	...
	//  Azul celeste // verde   //amarillo //rojo   // violeta  //azul verdoso //azul claro horario especial
	var colors = $.fn.calendar.colors = ['#2C8FC9', '#9CB703', '#F5BB00', '#FF4A32', '#B56CE2', '#45A597', '#9bccf7'];
