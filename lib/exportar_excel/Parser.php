...
*/
  function _convert($token)
    ...
    elseif(isset($this->ptg[$token])) // operators
        {
...
*
* @param $cell The cell to be packed
*/
...
*/
  function match($token)
    ...
    switch($token)
        ...
        default:
	    // if it's a reference
            if(preg_match("/^[A-Z][0-9]+$/i",$token) and
	       !preg_match("/[0-9]/",$this->_lookahead) and
            ...
            elseif(preg_match("/^[A-Z]?[A-Z][0-9]+:[A-Z]?[A-Z][0-9]+$/i",$token) and
		...
		return($token);
            ...
            // if it's a function
            elseif(preg_match("/^[A-Z0-9À-Ü\.]+$/i",$token) and ($this->_lookahead == "("))
	        {
		return($token);
		}
            return(0);
...
*/
  function _fact()
    ...
    // if it's a reference
    if (preg_match("/^[A-Z][0-9]+$/i",$this->_current_token))
        {
    ...
    // if it's a range
    elseif (preg_match("/^[A-Z]?[A-Z][0-9]+:[A-Z]?[A-Z][0-9]+$/i",$this->_current_token) or
        preg_match("/^[A-Z]?[A-Z][0-9]+\.\.[A-Z]?[A-Z][0-9]+$/i",$this->_current_token))
        {
        $result = $this->_current_token;
        $this->_advance();
        return($result);
    ...
    elseif (preg_match("/^[A-Z0-9À-Ü\.]+$/i",$this->_current_token))
        {
        $result = $this->_func();
        return($result);
        }
