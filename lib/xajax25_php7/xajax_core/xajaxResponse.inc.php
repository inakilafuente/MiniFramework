...
//SkipDebug
					if (htmlspecialchars($sInnerKey, ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401, 'UTF-8') != $sInnerKey) {
						$objLanguageManager = xajaxLanguageManager::getInstance();
						trigger_error(
...
//EndSkipDebug
		...
		if ($bNoOpenCDATA) {
			...
			if ($bNoCloseCDATA) {
				$bSpecialChars = (htmlspecialchars($sData, ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401, 'UTF-8') != $sData);
				if ($bSpecialChars)
					$nCDATA = 1;
			} else
				$nCDATA = 2;
