...
class PHPMailer {
    ...
    protected function AddAnAddress($kind, $address, $name = '') {
        ...
        if ($kind != 'Reply-To') {
            if (!isset($this->all_recipients[strtolower((string)$address)])) {
                array_push($this->$kind, array($address, $name));
                $this->all_recipients[strtolower((string)$address)] = true;
                return true;
            }
        } else {
            if (!array_key_exists(strtolower((string)$address), $this->ReplyTo)) {
                $this->ReplyTo[strtolower((string)$address)] = array($address, $name);
                return true;
            }
        }
        return false;
    ...
    public function WrapText($message, $length, $qp_mode = false) {
        ...
        $is_utf8 = (strtolower((string)$this->CharSet) == "utf-8");
        ...
        $line = explode((string)$this->LE, (string)$message);   // Magic. We know FixEOL uses $LE
        $message = '';
        for ($i = 0 ;$i < count($line); $i++) {
            $line_part = explode(' ', (string)$line[$i]);
            $buf = '';
    ...
    public function Base64EncodeWrapMB($str, $lf=null) {
        ...
        $offset = $avgLength = floor((float)$length * $ratio * .75);
        ...
        foreach($lines as $line){
            ...
            for($i = 0; $i < $linlen; $i++) {
                ...
                if ( $dec == 32 ) {
                    ...
                    $h2 = (integer)floor((float)$dec/16);
                    $h1 = (integer)floor((float)$dec%16);
    ...
    public function EncodeQP($string, $line_max = 76, $space_conv = false) {
        ...
        $fp = fopen('php://temp/', 'r+');
        $string = preg_replace('/\r\n?/', $this->LE, (string)$string); //Normalise line breaks
        ...
        $out = preg_replace('/^\./m', '=2E', (string)$out); //Encode . if it is first char on a line, workaround for bug in Exchange
        fclose($fp);
        return $out;
    }
    ...
    public function EncodeQ($str, $position = 'text') {
        ...
        if (preg_match_all("/[{$pattern}]/", $encoded, $matches)) {
            foreach (array_unique($matches[0]) as $char) {
                $encoded = str_replace($char, '=' . sprintf('%02X', ord((string)$char)), $encoded);
            }
        }
    ...
    public function AddCustomHeader($name, $value=null) {
        if ($value === null) {
            ...
            $this->CustomHeader[] = explode(':', (string)$name, 2);
        } else {
            ...
            foreach($images[2] as $i => $url) {
                ...
                if (!preg_match('#^[A-z]+://#', $url)) {
                    ...
                    if ( $this->AddEmbeddedImage($basedir.$directory.$filename, md5($url), $filename, 'base64', $mimeType) ) {
                        $message = preg_replace("/".$images[1][$i]."=[\"']".preg_quote($url, '/')."[\"']/Ui", $images[1][$i]."=\"".$cid."\"", (string)$message);
                    }
                }
            }
        }
    ...
    public static function _mime_types($ext = '') {
        ...
        return (!isset($mimes[strtolower((string)$ext)])) ? 'application/octet-stream' : $mimes[strtolower((string)$ext)];
    }
    ...
    public function DKIM_HeaderC($s) {
        ...
        foreach ($lines as $key => $line) {
            list($heading, $value) = explode(":", (string)$line, 2);
            $heading = strtolower((string)$heading);
            $value = preg_replace("/\s+/", " ", (string)$value) ; // Compress useless spaces
    ...
    public function DKIM_Add($headers_line, $subject, $body) {
        ...
        $headers              = explode((string)$this->LE, (string)$headers_line);
        $from_header          = "";
        $to_header            = "";
        ...
        $DKIMb64  = base64_encode((string)(pack("H*", sha1($body)))) ; // Base64 of packed binary SHA-1 hash of body
