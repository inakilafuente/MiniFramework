...
if (!(file_exists($nombreFichero))):
    ...
    for ($numFila = 2; $numFila <= $ultimaFila; $numFila++):
        ...
        /* COMPROBACIONES */
        // COMPRUEBO QUE LA CLAVE NO ESTÉ VACÍA
        if ($descripcionPep == ""):
            $strKo .= $auxiliar->traduce("Línea", $administrador->ID_IDIOMA) . " $numFila. ";
            $strKo .= $auxiliar->traduce("El Descripcion PEP esta vacio", $administrador->ID_IDIOMA) . " \n";
            $filasKo++;
            $errorLinea = true;
        endif;
        // COMPRUEBO QUE EL CAMPO PDA NO ESTÉ VACÍO Y QUE TENGA UN VALOR ADMISIBLE
        if ($bajaPep == ""):
            $strKo .= $auxiliar->traduce("Línea", $administrador->ID_IDIOMA) . " $numFila. ";
            $strKo .= $auxiliar->traduce("El campo Baja esta vacio", $administrador->ID_IDIOMA) . " \n";
            $filasKo++;
            $errorLinea = true;
        elseif (strtolower((string)$bajaPep) != "si" && strtolower((string)$bajaPep) != "no" && $bajaPep != "1" && $bajaPep != "0" && strtolower((string)$bajaPep) != "yes" && strtolower((string)$bajaPep) != "sí"):
            $strKo .= $auxiliar->traduce("Línea", $administrador->ID_IDIOMA) . " $numFila. ";
